...
const PROMPT_CAPABILITY_DEFINITIONS: Record<PromptCapabilityId, PromptCapabilityDefinition> = {
  ...
  characters: {
    label: '角色图鉴',
    systemNote: '可以使用角色姓名、定位、描述、标签等角色信息。角色行为必须由过往经历+当前利益+性格底色共同驱动，严禁反派降智或主角圣母。配角不能是工具人，必须有算盘、恐惧、筹码、误判与反扑。人设防崩：任何反常行为必须用前文铺垫解释，不能靠"突然顿悟"或"心软"搪塞。',
    userRule: '角色行为和新增角色必须尽量嵌入现有角色网络，避免孤立路人。反派和配角必须基于其已知信息和利益行动，不能为了推剧情而降智。'
  },
  relations: {
    label: '关系与组织',
    systemNote: '可以使用组织、角色关系、成员归属、阵营立场等结构化关系信息。配角不能是工具人，必须有算盘、恐惧、筹码、误判与反扑。冲突必须由利益驱动，不能是心软或圣母。角色关系的改变必须有事件驱动和铺垫，不能跨越式发展。',
    userRule: '如果关系、组织或归属会影响冲突、措辞或行动，优先把这些因素纳入输出。冲突必须由利益驱动，配角必须有反扑，不能站着等死。'
  },
  inspiration: {
    label: '灵感模块',
  ...
  chapters: {
    label: '章节编辑器',
    systemNote: '可以使用当前章节标题、摘要、状态、正文、相邻章节和选中文本。续写或改写前必须确认最近章节中的人物状态、已公开情报、已动用资源和未回收伏笔，确保因果连续。改写边界：润色只改表达不改事实，改写可改叙述但保留核心事实，重写可重构场景但不改主设定，续写只向前推进不反改前文。',
    userRule: '涉及正文时优先输出可直接插入、替换或回写章节的结果，而不是泛泛解释。续写必须紧接现有正文自然推进，保持语气、节奏和剧情方向一致。'
  },
  analysis: {
    label: '章节分析',
    systemNote: '可以围绕节奏、张力、连续性、风险和修正动作输出分析。审查维度包括：人物OOC、时间线断裂、利益链不成立、配角工具人化、爽点虚化、台词失真、语言重复、设定冲突、信息越界、伏笔失管、词汇疲劳。优先修根因，不做表面润色。',
