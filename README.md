...
仓库内置 GitHub Actions 工作流 `.github/workflows/build-macos.yml`：

- `workflow_dispatch`：手动触发构建
- `v*` tag push：版本标签触发构建
- 相关打包配置变更的 PR：验证 macOS 打包

工作流分别在 `macos-15` 与 `macos-15-intel` runner 上构建 Apple Silicon (`arm64`) 和 Intel (`x64`) 产物，并上传到 Actions artifacts。当前配置使用 ad-hoc 签名且未做公证；如需正式发布，需补充 `CSC_LINK`、`CSC_KEY_PASSWORD`、`APPLE_ID`、`APPLE_APP_SPECIFIC_PASSWORD`、`APPLE_TEAM_ID` 等 secrets 并启用 Developer ID 签名/公证流程。
